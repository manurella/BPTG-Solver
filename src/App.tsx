...
export default function App() {
  ...
  const handleKey = useCallback(
    (e: KeyboardEvent) => {
      ...
      if (e.key === 'End' && solution) { e.preventDefault(); setMoveIndex(solution.moves.length - 1); }
    },
  ...
  const boardPanel = (
    <main className="flex flex-1 flex-col items-center justify-center gap-5 overflow-auto p-4 md:p-6">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2">
        ...
        <button onClick={handleSolve} disabled={isSolving || photocards.length === 0}
    ...
    <div className="flex h-dvh flex-col overflow-hidden" style={{ backgroundColor: 'var(--color-bp-black)' }}>
      ...
      <header className="flex shrink-0 items-center justify-between border-b px-4 py-2 md:px-6 md:py-3"
        ...
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-base font-black tracking-tight md:text-lg" style={{ color: 'var(--color-bp-text)' }}>
            BPTG <span style={{ color: 'var(--color-bp-pink)' }}>Schedule</span>{' '}
      ...
      <div className="hidden md:flex min-h-0 flex-1">
        <aside className="w-56 shrink-0 overflow-y-auto border-r" style={{ borderColor: 'var(--color-bp-border)' }}>
          <PhotocardPanel />
        </aside>
        {boardPanel}
        <aside className="w-72 shrink-0 overflow-y-auto border-l" style={{ borderColor: 'var(--color-bp-border)' }}>
          <SolutionPlayer />
        </aside>
      </div>
          ...
          {(['cards', 'board', 'solution'] as MobileTab[]).map((tab) => (
            <button key={tab} onClick={() => setMobileTab(tab)}
              ...
              style={{
                ...
                borderTop: mobileTab === tab ? '2px solid var(--color-bp-pink)' : '2px solid transparent',
              }}>
